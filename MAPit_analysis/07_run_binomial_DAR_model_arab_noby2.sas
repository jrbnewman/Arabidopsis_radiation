...
proc glimmix data=data_for_models;
   ...
   model methyl_c / total_c = group|units / dist=binomial link=logit ddfm=kr chisq   oddsratio(diff=all);
...
data wgbslocA.bin3_dar_pred2_all;   set gmxout; run;
data wgbslocA.bin3_dar_anova2_all;   set anova; run;
data wgbslocA.bin3_dar_contrasts2_all;   set con; run;
data wgbslocA.bin3_dar_estimates2_all;   set est; run;
data wgbslocA.bin3_dar_fitstats2_all;   set fits; run;
data wgbslocA.bin3_dar_odds2_all;   set odds; run;
