...
data dmc_per_dmr2;
  ...
  if region_length >= 50 then flag_length_ge_50bp=1; else flag_length_ge_50bp=0;
...
proc freq data=dmc_per_dmr2 noprint;
  where flag_num_sites_ge5=1;
  tables comparison*site_type*flag_length_ge_50bp / out=check1;
...
proc multtest inpvalues(probf)=binomial2region fdr out=binom_fdr_ge5_50bp noprint;
  where flag_num_sites_ge5=1 and flag_length_ge_50bp=1;
  by comparison site_type;
run;
...
data flag_fdr_ge5_50bp;
set binom_fdr_ge5_50bp;
if fdr_p = . then flag_fdr05=.;
else if fdr_p < 0.05 then flag_fdr05=1;
else  flag_fdr05=0;
if mean_methyl_diff >= 0.1 then flaG_methdiff10=1; else flag_methdiff10=0;
...
proc freq data=flag_fdr_ge5 noprint;
  ...
  tables comparison*site_type*flag_length_ge_50bp / out=c1;
run;
...
proc freq data=flag_fdr_ge5_50bp noprint;
   where flag_methdiff10=1 and flag_fdr05=1; 
  tables comparison*site_type*flag_length_ge_50bp / out=c3;
run;
...
/* Binomial DMRs with >10% diff:
 ...
 0Gy_vs_1Gy     CHH           1         11746    30.3828
               ...
               site_    flag_length_
 comparison    type       ge_50bp      COUNT    PERCENT
...
0Gy_vs_1Gy     CHH           1         12358    63.8096
 ...
 Binomial DMRs with >10% diff and min 50bp in length:
...
data wgbsA.results_by_dmr_5sites_50bp;
   set flag_fdr_ge5_50bp;
run;
...
Binomial DMRs with >10% diff and min length of 50bp:
...
data wgbsA.results_by_dar_5sites_50bp;
   set flag_fdr_ge5_50bp;
run;
