...
data dmc;
  ...
  if FET_FDR_P = . then flag_fdr05=.;
   ...
   if FET_FDR_P = . then flag_fdr01=.;
  else if FET_FDR_P < 0.01 then flag_fdr01=1;
  else flag_fdr01=0;
  if FET_FDR_P = . then flag_fdr001=.;
  else if FET_FDR_P < 0.001 then flag_fdr001=1;
  else flag_fdr001=0;

  if flag_fdr05=1 and flag_meth_diff_10perc=1 then flag_fdr05_10perc=1;
  else flag_fdr05_10perc=0;

if flag_fdr01=1 and flag_meth_diff_10perc=1 then flag_fdr01_10perc=1;
  else flag_fdr01_10perc=0;
if flag_fdr001=1 and flag_meth_diff_10perc=1 then flag_fdr001_10perc=1;
  else flag_fdr001_10perc=0;
...
proc means data=dmc2region noprint;
  ...
  var start_pos stop_pos methyl_TRT methyl_CTL methyl_diff
      ...
      flag_fdr01 flag_fdr01_10perc
      flag_fdr001 flag_fdr001_10perc
      ;
  output out=meth_summary_by_region 
         ...
         sum(flag_fdr01)=num_sites_FDR01
         sum(flag_fdr01_10perc)=num_sites_FDR01_diff_10perc
         sum(flag_fdr001)=num_sites_FDR001
         sum(flag_fdr001_10perc)=num_sites_FDR001_diff_10perc;
...
data wgbslocA.results_by_DMR_annot_v2;
...
data dac;
  ...
  if FET_FDR_P_TRT_100U_0U=. then flag_TRT_fdr01=.;
  else if FET_FDR_P_TRT_100U_0U < 0.01 then flag_TRT_fdr01=1;
  else flag_TRT_fdr01=0;

  if FET_FDR_P_CTL_100U_0U=. then flag_CTL_fdr01=.;
  else if FET_FDR_P_CTL_100U_0U < 0.01 then flag_CTL_fdr01=1;
  else flag_CTL_fdr01=0;


  if FET_FDR_P_TRT_100U_0U=. then flag_TRT_fdr001=.;
  else if FET_FDR_P_TRT_100U_0U < 0.001 then flag_TRT_fdr001=1;
  else flag_TRT_fdr001=0;

  if FET_FDR_P_CTL_100U_0U=. then flag_CTL_fdr001=.;
  else if FET_FDR_P_CTL_100U_0U < 0.001 then flag_CTL_fdr001=1;
  else flag_CTL_fdr001=0;


   if flag_TRT_fdr05=1 or flag_CTL_fdr05=1
     then flag_FDR05_CTL_or_TRT=1;
  ...
  if flag_FDR05_CTL_or_TRT=1 and flag_meth_diff_20perc=1 then flag_fdr05_20perc=1;
   ...
   if flag_TRT_fdr01=1 or flag_CTL_fdr01=1
     then flag_FDR01_CTL_or_TRT=1;
     else flag_FDR01_CTL_or_TRT=0;
  if flag_FDR01_CTL_or_TRT=1 and flag_meth_diff_20perc=1 then flag_fdr01_20perc=1;
  else flag_fdr01_20perc=0;

   if flag_TRT_fdr001=1 or flag_CTL_fdr001=1
     then flag_FDR001_CTL_or_TRT=1;
     else flag_FDR001_CTL_or_TRT=0;
  if flag_FDR001_CTL_or_TRT=1 and flag_meth_diff_20perc=1 then flag_fdr001_20perc=1;
  else flag_fdr001_20perc=0;
run;
...
proc means data=dac2region noprint;
  ...
  flag_CTL_fdr01 
  flag_FDR01_CTL_or_TRT flag_TRT_fdr01
  flag_fdr01_20perc   

  flag_CTL_fdr001
  flag_FDR001_CTL_or_TRT flag_TRT_fdr001
  flag_fdr001_20perc 
  ...
  output out=meth_summary_by_region 
         ...
         sum(flag_CTL_fdr01)=num_sites_FDR01_CTL
         sum(flag_FDR01_CTL_or_TRT)=num_sites_FDR01_CTL_or_TRT
         sum(flag_TRT_fdr01)=num_sites_FDR01_TRT
         sum(flag_fdr01_20perc)=num_sites_FDR01_diff_20perc
	 
         sum(flag_CTL_fdr001)=num_sites_FDR001_CTL
         sum(flag_FDR001_CTL_or_TRT)=num_sites_FDR001_CTL_or_TRT
         sum(flag_TRT_fdr001)=num_sites_FDR001_TRT
         sum(flag_fdr001_20perc)=num_sites_FDR001_diff_20perc
...
data wgbslocA.results_by_DAR_annot_v2;
  set region_meth_binom_annot;
run;
