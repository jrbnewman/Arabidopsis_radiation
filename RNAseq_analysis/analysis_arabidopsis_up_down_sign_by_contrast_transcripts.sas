...
data estimates;
  ...
  keep transcript_id label estimate;
run;

data contrasts;
  set rs.arab_xs_cntrs_constr_non;
  keep transcript_id label ProbF;
run;

proc sort data=estimates;
   by transcript_id label;
...
keep transcript_id sign sign_label;
run;

proc sort data= sign_varname;
   by transcript_id sign_label;
...
data rs.arab_sign_by_contrast_transcript;
   set sign_sbys;
   drop _NAME_;
run;
