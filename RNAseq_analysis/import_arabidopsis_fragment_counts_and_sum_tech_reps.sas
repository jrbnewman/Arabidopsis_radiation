...
libname tair "!PATCON/useful_arabidopsis_data/TAIR10/sas_data";
 ...
 /* Import coverage counts for fragments and sum tech reps */

    data WORK.FRAGMENT_COUNTS    ;
    %let _EFIERR_ = 0; /* set the ERROR detection macro variable */
    infile
'!PATCON/arabidopsis/alignment_output/coverage_counts_fragments_arab.csv' delimiter = ',' MISSOVER DSD
...
proc sort data=design nodup;
  by sample_id;
proc sort data=fragment_counts;
  by sample_id;
run;

data counts_w_key;
  merge design (in=in1) fragment_counts (in=in2);
  by sample_id;
  if in1 and in2;
run;

proc sort data=counts_w_key;
   by sample_number treatment time replicate fragment_id;
...
data rs.arab_fragment_counts_w_key;
