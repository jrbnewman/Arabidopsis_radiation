...
%macro importHOMER(dataIN,dataOUT);
    ...
    infile "!PATCON/arabidopsis_wgbs_cold/analysis_output/HOMER_annotation/&dataIN." delimiter='09'x MISSOVER DSD lrecl=32767 firstobs=2 ;
...
%importHOMER(all_windows_for_HOMER_annotated.txt,windows_annot);
%importHOMER(all_superwindows_for_HOMER_annotated.txt,superwindows_annot);
...
data cold.homer_annotations_superwindows;
   set superwindows_annot;
   length geneID $20.;
   ...
   rename regionID=superwindowID;
run;

data cold.homer_annotations_windows;
   set windows_annot;
   length geneID $20.;
   if count(annotation,"promoter") >= 1 then flag_promoter=1; else flag_promoter=0;
   if count(annotation,"exon") >= 1 or count(annotation,"exon") >= 1 then flag_genebody=1; else flag_genebody=0;
