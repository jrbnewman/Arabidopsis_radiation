...
data meth_data;
  set arabMAP.methylation_data_gc arabMAP.methylation_data_cg_chg_chh;
  keep site_type chr stop_pos treatment units rep total_C ;
run;
...
proc means data=meth_data noprint;
  var total_C;
  output out=total_C_all_samples mean=mean stddev=sd min=min q1=q1 p5=p5 p10=p10 median=median q3=q3 p90=p90 p95=p95 max=max;
run;
...
proc means data=meth_data noprint;
  ...
  var total_C;
  output out=sum_C sum=;
run;

data sites;
  set sum_C;
  flag=1;
  keep site_type chr stop_pos flag;
run;

proc sort data=sites nodup;
  by chr stop_pos site_type;
proc transpose data=sites out=sites_sbys;
  by chr stop_pos;
  id site_type;
  var flag;
run;
...
proc freq data=sites_sbys noprint;
  tables CG*CHG*CHH*GC / out=site_ctabs;
proc print data=site_ctabs;
run;


proc means data=sum_C noprint;
  var total_C;
  output out=total_C_all_samples2 mean=mean stddev=sd min=min q1=q1 p5=p5 p10=p10 median=median q3=q3 p90=p90 p95=p95 max=max;
run;

proc means data=sum_C noprint;
  where total_C >= 10;
  var total_C;
  output out=total_C_all_samples3 mean=mean stddev=sd min=min q1=q1 p5=p5 p10=p10 median=median q3=q3 p90=p90 p95=p95 max=max;
run;


data sites2keep;
  set arabMAP.results_by_dmc_annot arabMAP.results_by_dac_annot;
  keep site_type chr stop_pos;
run;

proc sort data=sites2keep nodup;
  by site_type chr stop_pos;
proc sort data=sum_C;
  by site_type chr stop_pos;
run;

data sum_C_2;
  merge sum_C (in=in1) sites2keep (in=in2);
  by site_type chr stop_pos;
  if in1 and in2;
run;
...
proc sort data=sites2 nodup;
  by chr stop_pos site_type;
proc transpose data=sites2 out=sites2_sbys;
  by chr stop_pos;
  id site_type;
  var flag;
run;

proc freq data=sites2_sbys noprint;
  tables CG*CHG*CHH*GC / out=site_ctabs;
proc print data=site_ctabs;
run;
...
proc means data=sum_C noprint;
  where total_C >= 10;
  var total_C;
  output out=total_C_all_samples4 mean=mean stddev=sd min=min q1=q1 p5=p5 p10=p10 median=median q3=q3 p90=p90 p95=p95 max=max;
run;



proc means data=sum_C_2 noprint;
  where total_C > 10;
  var total_C;
  output out=total_C_all_samples5 mean=mean stddev=sd min=min q1=q1 p5=p5 p10=p10 median=median q3=q3 p90=p90 p95=p95 max=max;
run;
